                    ...
                    nodes = {
                        {
                            ...
                            nodes = {
                                UIBox_button({
                                    button = 'apply_filter_query_changes',
                                    label = { "Apply" },
                                    minw = 3,
                                    func =
                                    'filter_query_apply_button_UI'
                                }),
                            }
                        },
                        {
                            ...
                            nodes = {
                                UIBox_button({
                                    button = 'discard_filter_query_changes',
                                    label = { "Discard" },
                                    minw = 3,
                                    func =
                                    'filter_query_discard_button_UI'
                                }),
                            }
                        }
                    }
...
function tab_definition_function()
    ...
    -- seed_filter_ui.nodes[1].nodes[1].nodes[1].config.button = "apply_filter_query_changes"
    -- seed_filter_ui.nodes[1].nodes[2].nodes[1].config.button = "discard_filter_query_changes"

    -- print(seed_filter_ui.nodes[1])

    -- seed_filter_ui.nodes[1].nodes[2].nodes[1].nodes[1].config.button = 'apply_filter_query_changes'
...
function UIElement:click()
    if self.config.button and (not self.last_clicked or self.last_clicked + 0.1 < G.TIMERS.REAL) and self.states.visible and not self.under_overlay and not self.disable_button then
        ...
        if self.config.choice and deep_compare(filter_query, query) then
...
function G.FUNCS.options()
    ...
    if err or not deep_compare(filter_query, query) then
...
--     if not deep_compare(filter_query, parse_yaml(field.text)) then
--         print("Please submit or discard changes.")
--         print(field.text)
--         print(submitted_filter_query)
--         submit_status_node.config.text = "Please submit or discard changes."
...
-- local filter_query_changed = false

function G.FUNCS.apply_filter_query_changes(_initial)
    new_filter_query, err = parse_yaml(field.text)
    ...
    if err then
        ...
        submitted_filter_query = field.text
    end

    -- local filter_query = parse_yaml(field.text)
end

function G.FUNCS.discard_filter_query_changes(_initial)
    field.text = submitted_filter_query
    field:releaseMouse()
    ...
    new_filter_query = filter_query
end

function G.FUNCS.filter_query_apply_button_UI(e)
    ...
    if err or not deep_compare(filter_query, query) then
        e.config.button = 'apply_filter_query_changes'
...
function G.FUNCS.filter_query_discard_button_UI(e)
    local query, err = parse_yaml(field.text)
    ...
    if err or not deep_compare(filter_query, query) then
        e.config.button = 'discard_filter_query_changes'
